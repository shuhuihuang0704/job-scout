...
- If the user provides one or more job links without asking for discovery, analyze those links first and do not imply that a broader search was performed.
- Preserve the user's language and location context when expanding terms; include local-language and English variants only when they improve coverage.
...
8. Stop when the shortlist is sufficient for the user's stated decision, when the budget is reached, or when another round would only repeat already covered sources. State which stopping condition applied.
...
- Rank results in this order: hard-constraint fit, source and status evidence, freshness, then stated preferences. Do not hide a hard-constraint mismatch inside a score.
- If sources conflict, show the conflict and prefer the employer's current page or the most recent directly attributable posting.
...
If a required source cannot be opened, keep the direct URL, identify the source as unverified or incomplete, and separate visible facts from inference. Do not call a role `招聘中` based only on a search snippet.
...
- Never expose, request, or store access tokens, cookies, private job-board content, or recruiter contact details that are not already public and necessary for the user's question.
